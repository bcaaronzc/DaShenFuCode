...
public:
	...
	Mat process(Mat*srcImage);//处理单帧
	void openprovideo(Mat* dstImage);//打开处理过的视频的单帧
	void findnum(Mat*dstImage, Mat*tempImage, Mat*code, Mat*unit, int condition);//运算一个单元格的大小，两种模式，comlpex为小范围查找，simple是大范围查找
	//simple是不允许在中心行有任何噪音的，不然可能导致划分问题，但是运行时间短
	//complex是在中心反复选取ROI完成的，运行时间长但是更能克服边缘噪音
	int* getinf();//返回已经运算的一个单元格的长与宽
	//void findDigit(Mat* dstImage, Mat* tempImage, Ptr<ml::KNearest> knn, std::vector<Rect> rois);//识别并显示数字
};
...
private:
	...
	Preprocess pro;
	VideoCapture cap;
	bool istest, controlpoint;//控制测试的按钮，一键开启监控
	double delay;//延迟
	enum type{ simple, complex };
	int* p_unitsize;
	std::vector<Rect>* p_rect;
